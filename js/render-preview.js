...
function renderStandardItem(item, section) {
  ...
  const show = (key) => section.type === "courses" ? section.settings?.visibleFields?.[key] !== false : item.visibleFields?.[key] !== false && section.settings?.visibleFields?.[key] !== false;
  const hints = { experience: { title: "Seu cargo", organization: "Onde você trabalhou", city: "Cidade, Estado, País", description: "Conte o que você fazia e os resultados que alcançou." }, education: { title: "Nome do curso", organization: "Instituição de ensino", city: "Cidade, Estado, País", description: "Inclua informações relevantes sobre sua formação." }, courses: { title: "Nome do curso", organization: "Plataforma ou instituição", description: "Descreva o que você aprendeu." }, certifications: { title: "Nome da certificação", organization: "Organização emissora", description: "Inclua detalhes da credencial." }, projects: { title: "Nome do projeto", subtitle: "Subtítulo", description: "Explique o problema, sua participação e o resultado." }, languages: { title: "Idioma", description: "Observações" }, skills: { title: "Habilidade", description: "Contexto ou especialidade" }, interests: { title: "Interesse", description: "Conte um interesse relevante." }, achievements: { title: "Conquista", description: "Descreva o resultado alcançado." }, awards: { subtitle: "Tipo do prêmio", organization: "Empresa", city: "Local", description: "Descrição do prêmio" }, volunteering: { title: "Título", subtitle: "Subtítulo", city: "Local", description: "Descrição" }, references: { title: "Nome da pessoa", phone: "Telefone", organization: "Empresa", description: "Descrição" }, strengths: { title: "Título", subtitle: "Subtítulo", description: "Resumo" }, quote: { title: "Citação", description: "Uma frase que represente seu trabalho." }, timeline: { title: "Marco importante", description: "Conte um momento da sua trajetória." }, custom: { title: "Título", organization: "Subtítulo", description: "Adicione uma descrição." } }[section.type] || {};
  const period = section.type === "certifications" ? formatPeriod(item.date) : item.period || [formatPeriod(item.start || item.issued), item.current ? (item.degree ? "Em andamento" : "Atual") : formatPeriod(item.end || item.expires)].filter(Boolean).join(" — ") || formatPeriod(item.date) || "";
  ...
  const descriptionParagraphs = String(item.description || "").split(/\r?\n/).map((value) => value.trim()).filter(Boolean);
  const detailParts = [["degree", item.degree], ["level", section.type === "languages" ? "" : (item.customLevel || item.level)], ["city", section.type === "awards" || section.type === "volunteering" ? (item.city || "Local") : location], ["hours", show("hours") ? (item.hours || (section.type === "courses" ? "Carga horária" : "")) : ""], ["certificate", section.type === "languages" ? "" : item.certificate], ["phone", section.type === "references" && show("phone") ? (item.phone || "Telefone") : ""]].filter(([key, value]) => value && !(section.type === "courses" && key === "certificate"));
  const languageStars = section.type === "languages" && show("level") ? `<button type="button" class="language-stars ${Number(item.level) ? "" : "resume-placeholder"}" data-action="cycle-language-level" data-section-id="${sectionId}" data-item-id="${item.id}" aria-label="Alterar nível do idioma" title="Clique para alterar o nível">${Number(item.level) ? `${"★".repeat(Math.min(5, Number(item.level)))}${"☆".repeat(Math.max(0, 5 - Number(item.level)))}` : "Definir estrelas"}</button>` : "";
  ...
  return `<article class="resume-item"><div class="item-inline-tools"><button type="button" data-action="inline-delete-item" data-section-id="${sectionId}" data-item-id="${item.id}" aria-label="Excluir este item">Excluir Item</button></div>
    <div class="resume-item__top">${item.logo && show("logo") ? `<img class="resume-item-logo" src="${escapeHtml(item.logo)}" alt="Logotipo">` : ""}${show("title") ? inlineText((section.type === "awards" ? item.subtitle : item.title) || (section.type === "awards" ? hints.subtitle : hints.title) || "Novo item", { sectionId, itemId: item.id, key: section.type === "awards" ? "subtitle" : "title", tag: "h3" }) : ""}${!["strengths", "volunteering", "references"].includes(section.type) && (["projects", "certifications", "languages", "awards"].includes(section.type) ? show("date") : show("start")) ? `<button type="button" class="inline-date resume-meta" data-action="edit-inline-date" data-section-id="${sectionId}" data-item-id="${item.id}" aria-label="Editar período">${escapeHtml(period || (["courses", "certifications", "languages", "awards"].includes(section.type) ? "Data" : "Selecione o período"))}</button>` : ""}</div>
    ${["projects", "strengths", "volunteering"].includes(section.type) ? (show("subtitle") ? inlineText(item.subtitle || hints.subtitle || "Subtítulo", { sectionId, itemId: item.id, key: "subtitle", tag: "p", className: "resume-subtitle" }) : "") : (!["languages", "skills", "volunteering"].includes(section.type) && show("organization") ? inlineText(item.organization || hints.organization || "Organização", { sectionId, itemId: item.id, key: "organization", tag: "p", className: "resume-subtitle" }) : "")}
    ${(section.type === "courses" ? show("hours") : show("city")) && (detailParts.length || ["experience", "education"].includes(section.type)) ? `<p class="resume-description">${detailParts.length ? detailParts.map(([key, value]) => inlineText(value, { sectionId, itemId: item.id, key, className: "inline-detail" })).join(" · ") : inlineText(hints.city || "Cidade, Estado, País", { sectionId, itemId: item.id, key: "city", className: "inline-detail" })}</p>` : ""}${languageStars ? `<p class="resume-description">${languageStars}</p>` : ""}${show("description") ? (section.type === "experience" && item.descriptionBullets && descriptionParagraphs.length ? `<ul class="resume-list resume-description-list">${descriptionParagraphs.map((value) => `<li>${escapeHtml(value)}</li>`).join("")}</ul>` : inlineText(item.description || hints.description || "Adicione uma descrição.", { sectionId, itemId: item.id, key: "description", tag: "p", className: "resume-description" })) : ""}${(section.type === "projects" ? [["site",item.site,"Site"]] : [["projectLink",item.projectLink,"Projeto"],["link",item.link,"Link"],["site",item.site,"Site"]]).filter(([key,link]) => link && show(key) && !(section.type === "courses" && key === "link")).map(([key,link,label]) => inlineText(link, { sectionId, itemId: item.id, key, tag: "a", className: "resume-link" })).join(" ")}${section.type === "projects" && !item.site && show("site") ? inlineText("Site do projeto", { sectionId, itemId: item.id, key: "site", tag: "p", className: "resume-link resume-placeholder" }) : ""}${(item.images || []).length ? `<div class="project-images">${item.images.map((image) => `<img src="${escapeHtml(image)}" alt="Imagem do projeto">`).join("")}</div>` : ""}${custom}
  </article>`;
}
...
function renderSection(section) {
  ...
  if (section.type === "summary") body = inlineText(section.items[0]?.text || "", { sectionId: section.id, itemId: section.items[0]?.id, key: "text", tag: "p", className: "resume-summary" });
    ...
    const showSkillField = (key) => section.settings.visibleFields?.[key] !== false;
    const skills = section.items.filter((item) => item.subtitle || item.tags);
    body = `<div class="skill-groups">${skills.map((item) => { const tags = String(item.tags || "").split(",").map((tag) => tag.trim()).filter(Boolean); return `<div class="skill-group">${showSkillField("subtitle") ? inlineText(item.subtitle || "Subtítulo", { sectionId: section.id, itemId: item.id, key: "subtitle", tag: "p", className: "skill-group__subtitle" }) : ""}${showSkillField("tags") ? `<div class="skill-list">${(tags.length ? tags : ["Tags"]).map((tag) => `<span class="skill-tag${tags.length ? "" : " resume-placeholder"}">${escapeHtml(tag)}</span>`).join("")}</div>` : ""}</div>`; }).join("")}</div>`;
  }
  ...
  const sectionStyle = "";
  const titleStyle = "";
  const addMenu = [["experience","Experiência"],["education","Formação"],["courses","Curso"],["projects","Projeto"],["certifications","Certificação"],["languages","Idioma"],["skills","Habilidade"],["awards","Prêmios"],["volunteering","Voluntariado"],["references","Referências"],["strengths","Pontos fortes"],["custom","Categoria personalizada"]].map(([type,label]) => `<button type="button" data-action="inline-create-section" data-section-id="${section.id}" data-section-type="${type}">${label}</button>`).join("");
  const fieldLabels = { experience: [["title","Cargo"],["organization","Empresa"],["city","Local"],["start","Data"],["remote","Remoto"],["description","Descrição"],["projectLink","Links"],["logo","Logo"]], education: [["title","Curso"],["degree","Grau"],["organization","Instituição"],["city","Local"],["start","Data"],["description","Descrição"]], projects: [["title","Projeto"],["subtitle","Subtítulo"],["site","Site"],["date","Data"],["description","Descrição"]], awards: [["subtitle","Subtítulo"],["organization","Empresa"],["city","Local"],["date","Data"],["description","Descrição"]], certifications: [["title","Certificação"],["organization","Instituição"],["date","Data"],["description","Descrição"]], languages: [["level","Nível em estrelas"],["date","Data"],["description","Observações"]], skills: [["subtitle","Subtítulo"],["tags","Tags"]], strengths: [["title","Título"],["subtitle","Subtítulo"],["description","Resumo"]], volunteering: [["title","Título"],["subtitle","Subtítulo"],["city","Local"],["description","Descrição"]], references: [["title","Nome"],["phone","Telefone"],["organization","Empresa"],["description","Descrição"]], courses: [["title","Curso"],["organization","Instituição"],["start","Data"],["hours","Carga horária"],["description","Descrição"]] }[section.type] || [["title","Título"],["description","Descrição"]];
