...
export function migrateResumeData(data) {
  ...
  data.sections.forEach((section, index) => { section.visible ??= true; section.column ??= 1; section.order ??= index; section.settings ||= {}; section.customFields ||= []; section.items ||= []; section.items.forEach((item) => { item.customFields ||= []; item.visibleFields ||= {}; if (section.type === "header") item.photoSettings ||= { shape: "round", size: 84, borderWidth: 0, borderColor: "#2f6db1", position: "right" }; if (section.type === "experience") { item.remote ??= false; item.current ??= false; const oldActivities = Array.isArray(item.activities) ? item.activities.map((activity) => activity?.text || activity).filter(Boolean) : String(item.activities || "").split("\n").map((text) => text.trim()).filter(Boolean); if (oldActivities.length) { item.description = [item.description, ...oldActivities].filter(Boolean).join("\n"); item.descriptionBullets ??= true; } item.descriptionBullets ??= false; delete item.activities; item.technologies ||= ""; item.projectLink ||= ""; } if (section.type === "education") item.current ??= false; if (section.type === "projects") item.images ||= []; if (section.type === "languages") { item.level ||= ""; item.customLevel ||= ""; item.certificate ||= ""; } }); });
  ...
  data.sections.filter((section) => section.type === "skills").forEach((section) => section.items.forEach((item) => { item.subtitle ??= item.title || ""; item.tags ??= ""; }));
  return data;
}
