...
function renderAll() { renderEditor(getState(), activeTab); renderPreview(getState()); updateProgress(); updateZoom(); updateResumeSelector(); }
...
function createItemFor(section) {
  ...
  if (section.type === "skills") return { ...base, title: "Nova habilidade", level: "", category: "", description: "" };
  if (section.type === "languages") return { ...base, title: "Novo idioma", level: "", customLevel: "", certificate: "", description: "" };
  if (section.type === "experience") return { ...base, title: "Novo cargo", organization: "", city: "", state: "", country: "", remote: false, start: "", end: "", current: false, technologies: "", projectLink: "", description: "", activities: [] };
  ...
  if (section.type === "courses") return { ...base, title: "Novo curso", organization: "", hours: "", date: "", certificate: "", link: "", description: "" };
  if (section.type === "certifications") return { ...base, title: "Nova certificação", organization: "", issued: "", expires: "", noExpiry: false, credential: "", credentialLink: "", description: "" };
  if (section.type === "projects") return { ...base, title: "Novo projeto", technologies: "", github: "", site: "", projectLink: "", date: "", team: "", client: "", images: [], description: "" };
...
document.addEventListener("click", async (event) => {
  ...
  if (action === "inline-create-section") { const sourceId = event.target.dataset.sectionId; const type = event.target.dataset.sectionType; const titles = { experience: "Experiência profissional", education: "Formação", courses: "Cursos", projects: "Projetos", certifications: "Certificações", languages: "Idiomas", skills: "Habilidades", interests: "Interesses", achievements: "Conquistas", awards: "Prêmios", volunteering: "Voluntariado", references: "Referências", strengths: "Pontos fortes", quote: "Citação", timeline: "Meu tempo", custom: "Nova categoria" }; updateState((state) => { const index = state.sections.findIndex((section) => section.id === sourceId); const section = { id: createId("section"), type, title: titles[type], visible: true, column: state.sections[index]?.column || 1, order: 0, settings: {}, items: [], customFields: [] }; section.items.push(createItemFor(section)); state.sections.splice(index + 1, 0, section); normalizeOrder(state); }); renderAll(); autoSave(); toast("Seção adicionada e pronta para editar."); return; }
  ...
  if (action === "inline-section-settings") { const sectionId = event.target.dataset.sectionId; const section = getState().sections.find((entry) => entry.id === sectionId); if (!section) return; activeTab = "content"; renderEditor(getState(), activeTab); const card = document.querySelector(`[data-section-card="${sectionId}"]`); card?.classList.add("is-open"); card?.scrollIntoView({ behavior: "smooth", block: "start" }); toast("Configure os campos visíveis desta seção no painel avançado."); return; }
